...
function bavotasan_styles() {
	...
	?>
<style>
.container, .boxed #page, #main > .row, .home #primary article > .row, .archive #primary article > .row, .search #primary article > .row { max-width: <?php echo $bavotasan_theme_options['width']; ?>px; }
</style>
	<?php
}
...
function bavotasan_widgets_init() {
	...
	register_sidebar( array(
		...
		'description' => __( 'Widgetized area on the home page directly below the navigation menu. Specifically designed for 3 text widgets. Must be turned on in the Layout options on the Theme Options admin page.', 'ward' ),
		'before_widget' => '<aside id="%1$s" class="home-widget col-lg-3 %2$s">',
		'after_widget' => '</aside>',
